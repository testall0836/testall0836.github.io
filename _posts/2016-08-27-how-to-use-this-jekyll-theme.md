...
포스트 작성 방법							{#How-to-write-document}
------------------------------------

`LessOrMore/_posts`디렉터리에 새 파일을 만들면 쉽게 유지 관리 할 수 있도록 폴더를 만들고 폴더에 파일을 추가할 수 있습니다. 새 파일 붙여 넣기 정보를 다음과 같은 수정`titile`,`date`,`categories`,`tag`와 같은 관련 정보를 추가하고`* content {:toc}`为目录相关信息，在进行正文书写前需要在目录和正文之间输入至少2行空行。然后按照正常的Markdown语法书写正文。
